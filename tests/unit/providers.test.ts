...
describe("providersInternals.normalizeModelId", () => {
  test("normalizes prefixed ids and applies FS aliases", () => {
    ...
    assert.equal(providersInternals.normalizeModelId("gpt-5.5"), "gpt-5.4");
    assert.equal(
...
describe("providersInternals.shouldUseFsChatApi", () => {
  test("returns false for responses-compatible models", () => {
    assert.equal(providersInternals.shouldUseFsChatApi("gpt-5.5"), false);
    assert.equal(providersInternals.shouldUseFsChatApi("openai/gpt-5.5"), false);
